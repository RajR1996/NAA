...
namespace NAA.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        [AllowAnonymous]
        public ActionResult Go(string returnUrl)
        {
            ...
            else if ( (User.IsInRole("Applicant")) && (!User.IsInRole("Staff")) && ((!User.IsInRole("Admin")) ))
            {
                var appid = 0;
                var email = User.Identity.GetUserName();
                var userID = User.Identity.GetUserId();
                appid = _profileService.GetApplicantId(userID);
                if (appid != 0)
                {
                    System.Web.HttpContext.Current.Session["ApplicantId"] = appid;
                    return RedirectToAction("GetProfile", "Profile", new { id = appid });
                }
                else
                {
                    System.Web.HttpContext.Current.Session["UserId"] = userID;
                    System.Web.HttpContext.Current.Session["Email"] = email;
                    return RedirectToAction("CreateProfile", "Admin"/*, new { UserId = userID, Email = email }*/);
                }
            }
            else if (User.IsInRole("Staff"))
            {
                return RedirectToAction("GetUniversityList", "University");
            }
            else
            {
                return RedirectToLocal(returnUrl);
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    return RedirectToAction("CreateProfile", new { controller = "Admin", /*UserId = user.Id, Email = user.Email*/ });
                }
                AddErrors(result);
            }
        ...
        public async Task<ActionResult> ExternalLoginCallback(string returnUrl)
        {
            ...
            switch (result)
            {
                case SignInStatus.Success:
                    //return RedirectToLocal(returnUrl);
                    return RedirectToAction("Go", new { returnUrl = returnUrl });
                case SignInStatus.LockedOut:
                    return View("Lockout");
                case SignInStatus.RequiresVerification:
